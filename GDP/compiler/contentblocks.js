...
function modal(content, attributes) {
    var { title, subtitle, src, id, noFooter, button, } = attributes;

    subtitle =
        subtitle ? 
        `<div class="modal-subtitle">${subtitle}</div>` :
        ``;
    button = button ? button : 'Send them now >>';
...
function contentCard(content, attributes) {
    var preparation = attributes['preparation'];
    ...
    return `<div class="content-card">\n\n` +
                ...
                `<h3 class="content-card-title">` + 
                    (preparation ? `<a class="modal-link" data-modal-id="join-waitlist" href="#">${title}</a>`
                        : `<a href="${link}">${title}</a>`) + 
                `</h3>` + 
...
function _chapters(chapters, list) {
    return `<div class="series-toc-chapters">` +
                list.map(id => {
                    ...
                    var beginTag = 
                        chapter.preparation ?
                            `<a class="series-toc-chapter modal-link" data-modal-id="join-waitlist" href="#">`
                            :
                            `<a class="series-toc-chapter" href="${chapter.url}">`
                    return beginTag + 
...
function waitlistModal(content, attribute) {
    return modal('', {
...
addContentBlock('WaitlistModal', waitlistModal);
